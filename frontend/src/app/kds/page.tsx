...
export default function KdsPage() {
  ...
  useEffect(() => {
    // 1. Cargar comandas activas guardadas en SQLite
    ...
    // Registrar oyentes exclusivos de Cocina
  ...
  const handleUpdateStatus = (orderId: string, nextStatus: KdsOrder['status']) => {
    setOrders((prev) =>
      nextStatus === 'COMPLETED'
        ? prev.filter((ord) => ord.id !== orderId)
      ...
      <div className="p-3 bg-slate-50 border-t border-slate-100">
        ...
        {order.status === 'READY' && (
          <button
            ...
            className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white font-black rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer"
          >
            <CheckCircle2 className="w-4 h-4" />
            <span>Despachar / Servido</span>
          </button>
        )}
      </div>
    </div>

  ...
  return (
    <AdminGuard>
      <main className="min-h-screen bg-[#f3f6fb] text-slate-800 p-6 md:p-10 print:p-0 print:bg-white">
        ...
        <div className="max-w-7xl mx-auto space-y-8 print:hidden">
          ...
          <div className="bg-white p-7 rounded-[32px] border border-slate-200/80 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">
                  Corte de Caja y Métricas
                </h1>
                <p className="text-xs text-slate-500 mt-0.5">
                  Auditoría financiera, desglose de Efectivo / Tarjeta e
                  impuestos (IVA 16%).
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {/* Acceso directo a Gestión de Menú */}
              <Link
                href="/admin/menu"
                className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl text-xs uppercase tracking-wider transition-all"
              >
                  ...
                  {data.topProducts.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {data.topProducts.map((prod, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50/80 border border-slate-100"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-slate-400">
                                {prod.totalQty} unidades ordenadas
                              </div>
                            </div>
                          </div>
                      ...
                      <table className="w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {data.orders.map((order) => {
                            const itemSubtotal =
                              Math.round((order.total / 1.16) * 100) / 100;
                            const itemIva =
                              Math.round((order.total - itemSubtotal) * 100) /
                              100;
                            ...
                            return (
                              ...
                              >
                                <td className="py-3.5 font-black text-indigo-600">
                                  #{order.folio}
                                </td>
                                ...
                                <td className="py-3.5 text-slate-600 text-right font-mono">
                                  ${itemSubtotal}
                                </td>
                                <td className="py-3.5 text-slate-600 text-right font-mono">
                                  ${itemIva}
                                </td>
                                <td className="py-3.5 font-black text-slate-900 text-right font-mono">
                                  ${order.total}
                                </td>
                              </tr>
                            );
        ...
        {/* ============================================================== */}
        {/* 2. FORMATO IMPRESO EXCLUSIVO (ESTRUCTURA CONTABLE CORTE Z)     */}
        {/* ============================================================== */}
        {data && (
          <div className="hidden print:block w-full text-black font-sans text-[11px] p-2">
            <div className="mb-3 text-center">
              ...
              <p className="text-[10px] font-bold text-slate-700">
                CORTE DE CAJA DIARIO - FECHA: {data.date}
              </p>
            </div>
            ...
            <table className="w-full border-collapse border border-slate-500 text-[10px]">
              <thead>
                <tr className="bg-[#d9e1f2] text-slate-900 font-black text-center uppercase border-b border-slate-500">
                  <th className="border border-slate-500 px-2 py-1.5 w-24">
                    FECHA
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-28">
                    MESA Y HORA
                  </th>
                  <th className="border border-slate-500 px-3 py-1.5 text-left">
                    CONSUMO
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-20">
                    EFECTIVO
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-20">
                    TARJETA
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-24 text-right">
                    SUBTOTAL
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-28 text-right">
                    IVA TRASLADADO
                  </th>
                  <th className="border border-slate-500 px-2 py-1.5 w-24 text-right">
                    TOTAL
              ...
              <tbody>
                ...
                {/* Filas vacías para completar formato visual */}
                {Array.from({ length: minEmptyRows }).map((_, index) => (
                  <tr
                    key={`empty-${index}`}
                    className="border-b border-slate-300 h-6"
                  >
                    ...
                    <td className="border border-slate-400 px-2 py-1 text-center font-mono text-slate-400">
                      -
                    </td>
                    <td className="border border-slate-400 px-2 py-1 text-center font-mono text-slate-400">
                      -
                    </td>
                    <td className="border border-slate-400 px-2 py-1 text-right font-mono text-slate-400">
                      $ -
                    </td>
                    <td className="border border-slate-400 px-2 py-1 text-right font-mono text-slate-400">
                      $ -
                ...
                <tr className="bg-slate-100 font-black text-black border-t-2 border-slate-600">
                  ...
                  <td className="border border-slate-500 px-2 py-2 text-center font-mono font-black">
                    ${" "}
                    {data.summary.totalCash.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                  <td className="border border-slate-500 px-2 py-2 text-center font-mono font-black">
                    ${" "}
                    {data.summary.totalCard.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                  <td className="border border-slate-500 px-2 py-2 text-right font-mono font-black">
                    ${" "}
                    {data.summary.subtotal.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                  <td className="border border-slate-500 px-2 py-2 text-right font-mono font-black">
                    ${" "}
                    {data.summary.iva.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                  <td className="border border-slate-500 px-2 py-2 text-right font-mono font-black bg-slate-200">
                    ${" "}
                    {data.summary.totalRevenue.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                </tr>

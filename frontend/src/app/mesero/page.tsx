...
export default function MeseroPage() {
    ...
    useEffect(() => {
        ...
        // 2. Sockets en tiempo real
        setIsConnected(socket.connected);

        const onConnect = () => setIsConnected(true);
        const onDisconnect = () => setIsConnected(false);

        // Cuando Cocina marca un pedido como "READY"
        ...
        // Alerta de comensal solicitando mesero
        ...
        // Cuando otro mesero ya atendió la mesa
    ...
    // Marcar platillo como entregado al comensal
    const handleDeliverOrder = (orderId: string) => {
        setReadyOrders((prev) => prev.filter((o) => o.id !== orderId));
        socket.emit('order:status_update', { order_id: orderId, status: 'COMPLETED' });
    };
            ...
            <header className="sticky top-0 z-30 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-xs">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h1 className="text-base font-black text-slate-900 tracking-tight flex items-center gap-1.5">
                            NEXORA <span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-md font-bold">RUNNER / MESEROS</span>
                        </h1>
                        <p className="text-[11px] text-slate-500 font-medium">Despacho de Salón y Atención a Mesas</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    {!audioUnlocked && (
                        <button
                            onClick={unlockAudio}
                            className="p-2 bg-amber-50 border border-amber-300 rounded-xl text-amber-700 active:scale-95 transition-all"
                        >
                    ...
                    <div className="max-w-4xl mx-auto space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {alerts.map((alert) => (
                                ...
                                >
                                    <div>
                                        <div className="text-sm font-black text-amber-400">{alert.table}</div>
            ...
            <section className="max-w-4xl mx-auto p-4 space-y-4">
                ...
                {readyOrders.length === 0 ? (
                    <div className="py-20 text-center text-slate-400 space-y-2 bg-white rounded-3xl border border-slate-200 p-8 shadow-xs">
                        ...
                        <p className="font-bold text-slate-600 text-sm">No hay platillos pendientes de entrega</p>
                        <p className="text-xs text-slate-400">Los pedidos marcados en cocina como "Listos" aparecerán aquí inmediatamente.</p>
                    </div>
                ) : (
                            ...
                            >
                                <div>
                                    ...
                                    <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
                                        <div className="flex items-center gap-2">
                                            <span className="text-xl font-black text-slate-900">{order.table}</span>
                                                    ...
                                                    {item.modifiers && item.modifiers.length > 0 && (
                                                        <div className="text-xs text-slate-500 space-y-0.5 mt-0.5">
                                                            {item.modifiers.map((m, mIdx) => (
                                                                <span key={mIdx} className="inline-block bg-slate-100 px-1.5 py-0.5 rounded text-[11px] mr-1">
                                                                    {m.name}
                                                                </span>
                                                            ))}
                                                        </div>
